...
export function FilterTable({
  ...
  const applyFilters = () => {
    ...
    if (filters.tingkatPelanggaran) {
      filteredData = filteredData.filter(
        (item) => item.tingkatPelanggaran === filters.tingkatPelanggaran
      );
    }
    ...
    if (filters.jenisPelanggaran) {
      filteredData = filteredData.filter(
        (item) => item.jenisPelanggaran === filters.jenisPelanggaran
      );
    }
    ...
    if (filters.siswa) {
      filteredData = filteredData.filter(
        (item) => item.siswa.nama === filters.siswa
      );
    }
    ...
    if (filters.dateRange.start && filters.dateRange.end) {
      activeFilters.push({
        ...
        value: `${new Date(filters.dateRange.start).toLocaleDateString(
          "id-ID"
        )} - ${new Date(filters.dateRange.end).toLocaleDateString("id-ID")}`,
      });
    }
  ...
  const removeFilter = (filterKey: string) => {
    ...
    if (filterKey !== "tingkatPelanggaran" && newFilters.tingkatPelanggaran) {
      filteredData = filteredData.filter(
        (item) => item.tingkatPelanggaran === newFilters.tingkatPelanggaran
      );
    }

    if (filterKey !== "jenisPelanggaran" && newFilters.jenisPelanggaran) {
      filteredData = filteredData.filter(
        (item) => item.jenisPelanggaran === newFilters.jenisPelanggaran
      );
    }
    ...
    if (filterKey !== "siswa" && newFilters.siswa) {
      filteredData = filteredData.filter(
        (item) => item.siswa.nama === newFilters.siswa
      );
    }
