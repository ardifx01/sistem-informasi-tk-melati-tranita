  ...
  const formatOptions = [
    {
      ...
      label: "PDF Report",
      icon: FileText,
      desc: "Laporan formal dalam format PDF",
    },
    {
      value: "excel",
      label: "Excel Spreadsheet",
      icon: FileSpreadsheet,
      desc: "Data dalam format Excel (.xlsx)",
    },
    {
      value: "csv",
      label: "CSV Data",
      icon: File,
      desc: "Data mentah dalam format CSV",
    },
    {
      value: "print",
      label: "Print Report",
      icon: Printer,
      desc: "Cetak laporan langsung",
    },
  ];
  ...
  const generateCSV = (exportData: Pelanggaran[]) => {
    ...
    const rows = exportData.map((item) => {
      ...
      if (includeFields.siswa) {
        ...
        row.push(
          typeof item.siswa.kelas === "string"
            ? item.siswa.kelas
            : item.siswa.kelas?.nama || ""
        );
      }
      if (includeFields.tanggal)
        row.push(new Date(item.tanggal).toLocaleDateString("id-ID"));
        ...
        <div class="info">
          ...
          <p><strong>Periode:</strong> ${
            exportConfig.dateRange.start && exportConfig.dateRange.end
              ? `${new Date(exportConfig.dateRange.start).toLocaleDateString(
                  "id-ID"
                )} - ${new Date(exportConfig.dateRange.end).toLocaleDateString(
                  "id-ID"
                )}`
              : "Semua Data"
          }</p>
            ...
            <tr>
              ${
                exportConfig.includeFields.siswa
                  ? "<th>Nama Siswa</th><th>NIS</th><th>Kelas</th>"
                  : ""
              }
              ${exportConfig.includeFields.tanggal ? "<th>Tanggal</th>" : ""}
              ${
                exportConfig.includeFields.jenisPelanggaran
                  ? "<th>Jenis Pelanggaran</th>"
                  : ""
              }
              ${
                exportConfig.includeFields.tingkatPelanggaran
                  ? "<th>Tingkat</th>"
                  : ""
              }
              ${
          ...
          <tbody>
            ${exportData
              ...
              <tr>
                ...
                ${
                  exportConfig.includeFields.tanggal
                    ? `<td>${new Date(item.tanggal).toLocaleDateString(
                        "id-ID"
                      )}</td>`
                    : ""
                }
                ...
                ${
                  exportConfig.includeFields.tingkatPelanggaran
                    ? `<td><span class="tingkat-${item.tingkatPelanggaran.toLowerCase()}">${
                        item.tingkatPelanggaran
                      }</span></td>`
                    : ""
                }
                ${
                  exportConfig.includeFields.deskripsi
                    ? `<td>${item.deskripsi}</td>`
                    : ""
                }
                ${
                  exportConfig.includeFields.tindakan
                    ? `<td>${item.tindakan}</td>`
                    : ""
                }
                ${
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(exportConfig.includeFields).map(
                ([key, value]) => (
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <p className="text-sm text-gray-600">
              ...
              {getDataToExport().length} pelanggaran
            </p>
            {exportConfig.dateRange.start && exportConfig.dateRange.end && (
              <p className="text-sm text-gray-600">
